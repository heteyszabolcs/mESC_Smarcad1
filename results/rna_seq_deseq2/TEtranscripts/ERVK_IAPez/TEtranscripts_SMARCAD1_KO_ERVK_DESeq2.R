...
data <- read.table("TEtranscripts_SMARCAD1_KO_ERVK.cntTable",header=T,row.names=1)
...
dds <- DESeq(dds)
res <- results(dds)
write.table(res, file="TEtranscripts_SMARCAD1_KO_ERVK_gene_TE_analysis.txt", sep="\t",quote=F)
resSig <- res[(!is.na(res$padj) & (res$padj < 0.050000) &         (abs(res$log2FoldChange)> 0.000000)), ]
write.table(resSig, file="TEtranscripts_SMARCAD1_KO_ERVK_sigdiff_gene_TE.txt",sep="\t", quote=F)
